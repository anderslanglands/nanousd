...
/// 
/// @brief Iterators providing access to arrays of types than cannot be represented in C
///
/// @{
...
typedef struct nusd_asset_path_array_iterator_s* nusd_asset_path_array_iterator_t;
...
/// @param token Output pointer that will contain the current token if available.
/// 
...
/// @note The returned token is valid until the iterator is destroyed or advanced.
NANOUSD_API
bool nusd_token_array_iterator_next(nusd_token_array_iterator_t tokens, char const** token);
...
/// 
/// @note iterator must not be null.
/// @note This function returns the total size regardless of the current iteration position.
NANOUSD_API
...
/// 
/// @param tokens Valid token array iterator handle to destroy. Can be null (no-op).
/// 
/// @return NUSD_RESULT_OK on success
/// 
/// @note This function always returns NUSD_RESULT_OK.
...
/// @note It is safe to call this function with a null iterator handle.
NANOUSD_API
nusd_result_t nusd_token_array_iterator_destroy(nusd_token_array_iterator_t tokens);

/// Advances the token array iterator to the next token and returns its path.
/// 
/// @param asset_paths Valid asset_path array iterator handle.
/// @param asset_path Output pointer that will contain the current asset_path if available.
/// 
...
/// @note The returned asset_path is valid until the iterator is destroyed or advanced.
NANOUSD_API
...
/// 
/// @param asset_paths Valid asset_path array iterator handle to destroy. Can be null (no-op).
/// 
/// @return NUSD_RESULT_OK on success
/// 
/// @note This function always returns NUSD_RESULT_OK.
